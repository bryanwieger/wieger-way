...
Step 7 — Review dependent tasks after completion
  When this task reaches "Finalized", find every task that listed this one
  in its "Depends on" section. Review each dependent task to determine
  whether the way this task was completed changes how the dependent task
  should behave or be implemented. Update dependent task files if needed
